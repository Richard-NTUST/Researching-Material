# Meeting Notes

> **What should I record during a research meeting?**

## What?

### Meeting Information

| Item         | Description |
| ------------ | ----------- |
| Date         | 2026/07/02  |
| Participants | BMW 實驗室成員 + 短期實習交換生 |
| Topic        | 每個人的進度報告，含專題生 |

---

### 1. Review Pending Tasks

| Pending Task | Owner | Status                     | Remarks |
| ------------ | ----- | -------------------------- | ------- |
| 確認研究主題與下一步計畫 | Richard | [x] Done [ ] Pending [ ] Blocked | 先從學長姊的論文開始下手，要分清貢獻度 |

---

### 2. Discussion Topics

| Topic | Reference |
| ----- | --------- |
| 1. 實驗室會議與 Demo 規範 (全英文報告、直播錄音錄影、Demo 需關閉自身音訊防 echo、注意交接版本問題) | 會議口頭宣達 |
| 2. 實驗室資源與排版參考 (會議檔案放置於指定 Google Drive，過往資料洽 Ian 學長，GitHub 排版暫參考承諭學長) | [Google Drive](https://drive.google.com/drive/folders/1GV_zlQZgghiazh1a_ULu-EIskeqPv6VZ) |
| 3. Energy Saving 主題確立方向：檢視現有碩士論文尋找改進點，鎖定 RU power modeling | David 學長資料、Tobby & Sheryl 交接內容 |

---

### 3. New Action Items

| Task | Owner | Measurable Deliverable | Due Date | Evidence |
|---|---|---|---|---|
| 閱讀 David 學長資料與 YMA-Thesis | Richard | 1 份閱讀筆記，找出可改進之處作為專題貢獻 | 2026/07/09 | Daily Note 紀錄 |
| 完成 7/9 專題交接準備 | Richard | 1 份填妥空白欄位的 Research_Assignment.md | 2026/07/09 | GitHub 檔案連結 |
| 規劃 8-weekly plan | Richard | 1 份八週專題進度規劃表 | 待定 | GitHub 檔案連結 |

---
